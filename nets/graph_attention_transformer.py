...
# _AVG_NUM_NODES = 13.669001751313486
_AVG_NUM_NODES = 42.50108904548367

_AVG_DEGREE = 42.50108904548367
...
class NodeEmbeddingNetwork(torch.nn.Module):
    def __init__(self, irreps_node_embedding, max_atom_type=10, bias=True):
        super().__init__()
    ...
    def forward(self, node_atom):
        node_atom = node_atom.to(dtype=default_dtype)  # ensure correct dtype

        assert node_atom.shape[1] == self.max_atom_type, \
            f"Expected shape (*, {self.max_atom_type}), but got {node_atom.shape}"

        node_attr = node_atom  # already one-hot
        node_embedding = self.atom_type_lin(node_attr)

        return node_embedding, node_attr, node_attr 
...
#         '''
#         print(torch.abs(node_atom))
#         print(self.max_atom_type)
#         print(f"[DEBUG] node_atom.min()={node_atom.min()}, max={node_atom.max()}, max_atom_type={self.max_atom_type}")
...
class GraphAttentionTransformer(torch.nn.Module):
    def __init__(self,
        ...
        self.atom_embed = NodeEmbeddingNetwork(self.irreps_node_embedding, max_atom_type = self.irreps_node_input.num_irreps)
        ...
        #     # LinearRS(self.irreps_feature, o3.Irreps('1806x0e'), rescale=_RESCALE)) 
        #     )

        # self.head_linear = LinearRS(self.irreps_feature, self.irreps_feature, rescale=_RESCALE)
        # self.head_linear = FullyConnectedTensorProductRescaleSwishGate(
        #     self.irreps_feature, self.irreps_feature, self.irreps_feature, rescale=_RESCALE)
        # self.head_activation = Activation(self.irreps_feature, acts=[torch.nn.SiLU()])
        # self.head_tp = FullyConnectedTensorProductRescale(
        # self.irreps_feature, self.irreps_feature, o3.Irreps('301x0e+301x2e'), bias=True, rescale=_RESCALE)
        # self.head_tp = FullyConnectedTensorProductRescaleSwishGate(
        # self.irreps_feature, self.irreps_node_attr, o3.Irreps('301x0e+301x2e'), bias=True, rescale=_RESCALE)
        # self.head_tp = FullyConnectedTensorProductRescale(
        #     self.irreps_feature, self.irreps_feature, o3.Irreps('1806x0e'), bias=True, rescale=_RESCALE)
        self.scale_scatter = ScaledScatter(_AVG_NUM_NODES)

        self.TI_tp = o3.FullyConnectedTensorProduct(self.irreps_feature, self.irreps_node_attr, "300x0e+300x2e")
        # self.TI_tp = o3.FullyConnectedTensorProduct(self.irreps_feature, self.irreps_node_attr, "1800x0e")

        # self.TI0 = o3.Linear(self.irreps_feature, self.irreps_feature)
        # self.TI1 = o3.Linear(self.irreps_feature, self.irreps_feature)
    ...
    def forward(self, f_in, pos, batch, node_atom, **kwargs) -> torch.Tensor:
        ...
        # node_features = self.head_linear(node_features, node_features)
        # # node_features = self.head_activation(node_features)
        # outputs = self.head_tp(node_features, node_features)  # ← pass both feature and attr
        node_features=self.TI_tp(node_features,node_attr)
        
        outputs = scatter(node_features, batch, dim=0,reduce="mean").div(_AVG_NUM_NODES ** 0.5)

        # outputs = self.scale_scatter(node_features, batch, dim=0)
        # outputs1 = self.TI0(outputs)
        # outputs2 = self.TI1(outputs1)
...
#     num_atoms = 5
#     z = torch.randint(1, 4, (num_atoms,), dtype=torch.long)  # [1, 3)
#     z_shifted = z - 1                                        # [0, 2)
#     print("z:", z)
...
#     f_in = torch.nn.functional.one_hot(z_shifted, num_classes=3).float()

#     # Instantiate model with max_atom_type = 3
#     model = graph_attention_transformer_nonlinear_l2_e3(
#         irreps_in=Irreps("3x0e"),
#         radius=5.0,
#         num_basis=16,
#         max_atom_type=3
#     )
...
#     # Correctly pass the one-hot f_in and the shifted z to node_atom
#     with torch.no_grad():
#         # pred = model(f_in=data.x, pos=data.pos, batch=data.batch, node_atom=data.z,
#         #              edge_d_index=data.edge_d_index, edge_d_attr=data.edge_d_attr)
#         out = model(f_in=f_in, pos=pos, batch=batch, node_atom=z)

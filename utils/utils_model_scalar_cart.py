...
def visualize_layers(model):
    ...
    fig.savefig(f'../pngs/layer_viz.png')
    plt.close(fig)

def evaluate(model, dataloader, loss_fn_eval, loss_fn_mae_eval, device, out_dim=300, ncomp=6):
    """Evaluate model directly on Cartesian tensor outputs."""
    model.eval()
    loss_cumulative = 0.0
    ...
    with torch.no_grad():
        for d in dataloader:
            d = d.to(device)
            output = model(d)  # Shape: (batch, out_dim * ncomp)

            # Reshape output and target into (batch, out_dim, 3, 3)
            output_cart = output.view(d.y.shape[0], out_dim, ncomp)
            y_cart = d.y.view(d.y.shape[0], out_dim, ncomp)

            loss = loss_fn_eval(output_cart, y_cart)
            loss_mae = loss_fn_mae_eval(output_cart, y_cart)
...
def train(model, optimizer, dataloader_train, dataloader_valid,
          loss_fn, loss_fn_mae, loss_fn_eval, loss_fn_mae_eval,
          run_name, max_iter=101, scheduler=None, device="cpu",
          disable_tqdm=False, out_dim=300, ncomp=6, selected_components=None):

    model.to(device)
    start_time = time.time()

    try:
        model.load_state_dict(torch.load(f'../model/{run_name}.torch')['state'])
    except:
        results, history, s0, best_valid_loss = {}, [], 0, float('inf')
    else:
        results = torch.load(f'../model/{run_name}.torch')
    ...
    for step in range(max_iter):
        model.train()
        loss_cumulative, loss_cumulative_mae = 0.0, 0.0
        ...
        for batch_idx, d in enumerate(tqdm(dataloader_train, total=len(dataloader_train),
            ...
            # Reshape to Cartesian tensors
            output_cart = output.view(d.y.shape[0], out_dim, ncomp)
            y_cart = d.y.view(d.y.shape[0], out_dim, ncomp)

            loss = loss_fn(output_cart, y_cart).mean()
            loss_mae = loss_fn_mae(output_cart, y_cart).mean()
            ...
            loss_cumulative += loss.item()
            loss_cumulative_mae += loss_mae.item()
            # === Plot and save for first sample in batch ===
            if step % 2 == 0 and batch_idx == 0:
                # First sample
                pred = output_cart[0].detach().cpu().numpy()   # shape: (out_dim, 6)
                target = y_cart[0].detach().cpu().numpy()      # shape: (out_dim, 6)

                comp_labels = selected_components   # e.g. ["xx", "yy", "xz"]
                ncomp = len(comp_labels)

                # Auto grid layout: 1 row if few comps, 2 rows otherwise
                ncols = min(3, ncomp)
                ...
                for i, label in enumerate(comp_labels):
                    ax = axes[i]
                    ax.plot(pred[:, i], label="Prediction", alpha=0.9)
                    ax.plot(target[:, i], label="Ground Truth", alpha=0.9)
                    ax.set_title(f"Comp {label}")
                    if i == 0:
                        ax.legend()

                # Hide unused subplots (if any)
                for j in range(len(comp_labels), len(axes)):
                    axes[j].axis("off")

                fig.suptitle(f"Step {step} | Batch {batch_idx}", fontsize=14)
                fig.savefig(f"../pngs/pred_vs_gt_step{step}_batch{batch_idx}.png")
                plt.close(fig)
        end_time = time.time()
        wall = end_time - start_time

        valid_avg_loss = evaluate(model, dataloader_valid, loss_fn_eval, loss_fn_mae_eval, device, out_dim, ncomp)
        train_avg_loss = evaluate(model, dataloader_train, loss_fn_eval, loss_fn_mae_eval, device, out_dim, ncomp)

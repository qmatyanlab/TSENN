...
def evaluate(model, dataloader, loss_fn, loss_fn_mae, device):
    ...
    with torch.no_grad():
        for j, d in enumerate(dataloader):
            ...
            output = model(d)
            loss = loss_fn(output, y_unrolled).cpu()
            loss_mae = loss_fn_mae(output, y_unrolled).cpu()
...
def train(model, optimizer, dataloader_train, dataloader_valid, loss_fn, loss_fn_mae, run_name,
    ...
    try: 
        ...
        s0 = history[-1]['step'] + 1
        best_valid_loss = min(h['valid']['loss'] for h in history)  # Retrieve the best validation loss from history
    ...
    for step in range(max_iter):
        ...
        if step == checkpoint:
            ...
            if valid_avg_loss[0] < best_valid_loss:
                ...
                with open(f'../model/{run_name}_best.torch', 'wb') as f:

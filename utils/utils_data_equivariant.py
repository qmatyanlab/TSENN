...
# build data
def build_data(entry, column, scale_data, type_onehot, mass_onehot, dipole_onehot, radius_onehot, type_encoding, r_max=6.):
    # Extract structure data
    symbols = list(entry.symmetrized_structure.symbols).copy()
    positions = torch.from_numpy(entry.symmetrized_structure.positions.copy()).to(default_dtype)
    lattice = torch.from_numpy(entry.symmetrized_structure.cell.array.copy()).unsqueeze(0).to(default_dtype)

    # Neighbor list for edges
    edge_src, edge_dst, edge_shift = neighbor_list("ijS", a=entry.symmetrized_structure, cutoff=r_max, self_interaction=True)
    
    # Compute edge vectors
    edge_batch = torch.zeros(positions.shape[0], dtype=torch.long)[torch.from_numpy(edge_src)]
    edge_vec = (positions[torch.from_numpy(edge_dst)]
                - positions[torch.from_numpy(edge_src)]
                + torch.einsum('ni,nij->nj', torch.tensor(edge_shift, dtype=default_dtype), lattice[edge_batch]))

    edge_len = edge_vec.norm(dim=1).round(decimals=2)

    # Crystal system one-hot encoding
    crystal_systems = ["triclinic", "monoclinic", "orthorhombic", "tetragonal", "trigonal", "hexagonal", "cubic"]
    crystal_system_idx = crystal_systems.index(entry["crystal_system"].lower())
    crystal_system_onehot = torch.zeros(len(crystal_systems), dtype=default_dtype)
    crystal_system_onehot[crystal_system_idx] = 1.0

    # Convert rotation matrices to Wigner D-matrices
    rotation_matrices = torch.from_numpy(entry.rotation_matrices).to(default_dtype)
    if rotation_matrices.dim() == 2:  # Single matrix case
        rotation_matrices = rotation_matrices.unsqueeze(0)
    elif rotation_matrices.dim() != 3 or rotation_matrices.shape[-2:] != (3, 3):
        raise ValueError(f"Expected rotation_matrices of shape (N, 3, 3), got {rotation_matrices.shape}")

    irreps = Irreps("1x0e + 1x2e")  # Scalar + rank-2 tensor
    wigner_D_tensor = irreps.D_from_matrix(rotation_matrices)  # Shape: (N, 6, 6) where N is num rotations for this entry
    wigner_D_list = [wigner_D_tensor[i] for i in range(wigner_D_tensor.shape[0])]  # List of (6, 6) tensors
    # Build Data object
    data = tg.data.Data(
        pos=positions,
        lattice=lattice,
        symbol=symbols,
        x_mass=mass_onehot[[type_encoding[specie] for specie in symbols]],
        x_dipole=dipole_onehot[[type_encoding[specie] for specie in symbols]],
        x_radius=radius_onehot[[type_encoding[specie] for specie in symbols]],
        ...
        edge_vec=edge_vec,
        edge_len=edge_len,
        y=torch.from_numpy((entry[column] / scale_data)).unsqueeze(0).to(default_dtype),  # 'sph_coefs'
        crystal_system_onehot=crystal_system_onehot.unsqueeze(0),
        wigner_D=wigner_D_list  # Store as a top-level attribute, list of (6, 6) tensors
    )
    return data
...
def plot_spherical_harmonics_comparison(df, idx, column_name, title_prefix="", n=4):
    ...
    with sns.plotting_context("notebook", font_scale=2):
        ...
        for i, sample_idx in enumerate(idx[:n]):  # Use provided indices directly
            ...
            ax_target.tick_params(axis='x', rotation=0, labelsize=20)  # Ensures x-axis labels are horizontal
            ax_target.tick_params(axis='y', rotation=0, labelsize=20)  # Ensures y-axis labels are horizontal
            ...
            ax_pred.tick_params(axis='x', rotation=0, labelsize=20)  # Ensures x-axis labels are horizontal
            ax_pred.tick_params(axis='y', rotation=0, labelsize=20)  # Ensures x-axis labels are horizontal

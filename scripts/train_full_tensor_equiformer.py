...
import wandb
from nets.graph_attention_transformer import graph_attention_transformer_nonlinear_l2_e3, GraphAttentionTransformer
...
avg_deg = np.mean(n_train)
print("_AVG_DEGREE =", avg_deg)


def compute_avg_num_nodes(dataset):
    total_nodes = 0
    num_graphs = 0
    ...
    return total_nodes / num_graphs

avg_num_nodes = compute_avg_num_nodes(df.iloc[idx_train]['data'].values)
print("_AVG_NUM_NODES =", avg_num_nodes)
...
class equiformer_wrapper(nn.Module):
    def __init__(self, in_dim, em_dim, gat_model, reduce_output=False):
        super().__init__()
        self.gat_model = gat_model
        self.pool = reduce_output
    ...
    def forward(self, data: Union[tg.data.Data, Dict[str, torch.Tensor]]) -> torch.Tensor:
        # Embed z and x
        data.z = F.relu(self.em_z(data.z))  # expects data.z to be one-hot or float
        data.x = F.relu(self.em_x(data.x))  # expects data.x to be float

        # Forward through wrapped GAT model
        out = self.gat_model(f_in=data.x, pos=data.pos, batch=data.batch, node_atom=data.z)

        return out

gat_model = graph_attention_transformer_nonlinear_l2_e3(
    irreps_in=o3.Irreps("64x0e"),  # output dim of em_x
    radius=7,
    num_basis=128
)

# Wrap it
model = equiformer_wrapper(
    in_dim=INPUT_FEATURE_DIM,     # match data.z and data.x input size (e.g., 3 if 3-atom one-hot)
    em_dim=EMBEDDING_DIM,         # embedding dim to match "64x0e"
    gat_model=gat_model,
    reduce_output=True
)
...
dropout_prob = 0.2
wandb.init(
    project="Tensor Predict Equiformer",  # Change this to your project name

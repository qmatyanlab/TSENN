...
from utils.eatgnn_ori import train
import wandb
...
def main_worker(rank, world_size):
    ...
    if rank == 0:
        ...
        wandb.init(
            project="Tensor Predict Equiformer",
            name=f'with loss balancer',
            config={
                "max_iter": max_iter,
    ...
    train(model, opt, dataloader_train, dataloader_valid, loss_fn, loss_fn_mse, loss_fn_eval, loss_fn_mse_eval, 
          f"run_gpu_{rank}", max_iter=max_iter, scheduler=scheduler, device=device,
